...
#slide[
  ...
  #proof[
    ...
    sqrt(n)(hat(theta) - theta^*) 
      &= - (nabla^2 hat(L)(theta^*))^(-1) sqrt(n) nabla hat(L)(theta^*) + sqrt(n) O(||hat(theta) - theta^*||^2_2) \
      &approx - (nabla^2 L(theta^*))^(-1) sqrt(n) nabla hat(L)(theta^*).
    $
    ...
    #theorion-restate(
      ...
      render: 
        it 
          ...
          => block[#strong[#full-title.]#sym.space#emph(body)],
    )
    ...
    $
    $
    "and thus" sqrt(n) nabla hat(L)(theta^*) arrow.r^d cal(N)(0, "Cov"(nabla cal(l)((x,y), theta^*))).
    $
    By _the law of large numbers_, $nabla^2 hat(L)(theta^*)^(-1)$
  ]
]
